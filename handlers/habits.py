...
# ... (show_habit_card и другие хэндлеры до FSM без изменений)
...
async def track_habit_callback(callback: CallbackQuery, state: FSMContext):
    ...
    if db.track_habit(habit_id):
        await callback.answer("✅ Отлично! Привычка отмечена.", show_alert=True)
    else:
        await callback.answer("ℹ️ Эта привычка уже была отмечена сегодня.", show_alert=True)
    await show_habit_card(callback, habit_id, state)
...
async def delete_habit_start_handler(callback: CallbackQuery):
    ...
    await callback.message.edit_text("Вы уверены, что хотите удалить эту привычку? Это действие необратимо.", reply_markup=keyboard)
...
async def delete_habit_confirm_handler(callback: CallbackQuery):
    ...
    habits = db.get_user_habits(callback.from_user.id)
    if not habits:
        await callback.message.edit_text("У вас больше нет привычек.", reply_markup=None)
        return
    keyboard = kb.get_habits_pagination_kb(habits, page=0)
    await callback.message.edit_text("📋 **Ваши привычки:**", reply_markup=keyboard, parse_mode="Markdown")
...
async def process_habit_name(message: Message, state: FSMContext):
    ...
    await message.answer("Отлично! Теперь введите краткое описание или мотивацию:")
...
async def process_habit_description(message: Message, state: FSMContext):
    ...
    await message.answer("Выберите дни недели для этой привычки:", reply_markup=kb.get_days_of_week_kb())

# --- ИСПРАВЛЕНИЕ: Добавлен StateFilter ---
...
async def process_reminder_callback(callback: CallbackQuery, state: FSMContext):
    ...
    if callback.data == "reminder_yes":
        ...
        if user_timezone is None:
            ...
            await callback.message.answer("Укажите ваш часовой пояс (например, +3 или -5):")
        else:
            await state.set_state(HabitCreation.reminder_time)
            await callback.message.answer("Введите время для напоминания в формате ЧЧ:ММ:")
    else:
        data = await state.get_data()
        db.save_habit(telegram_user_id=callback.from_user.id, name=data['name'], description=data['description'], days=list(data.get('selected_days', [])))
        await state.clear()
...
async def process_timezone(message: Message, state: FSMContext):
    try:
        ...
        await state.set_state(HabitCreation.reminder_time)
        await message.answer(f"Отлично, ваш часовой пояс установлен: UTC{offset:+}.\n\nВведите время для напоминания:")
    except (ValueError, TypeError):
        await message.answer("Неверный формат. Введите число (например, +3 или -5).")
...
async def process_reminder_time(message: Message, state: FSMContext):
    if not re.match(r'^\d{2}:\d{2}$', message.text):
        ...
        return
    data = await state.get_data()
    db.save_habit(telegram_user_id=message.from_user.id, name=data['name'], description=data['description'], days=list(data.get('selected_days', [])), reminder_time=message.text)
    await state.clear()
    await message.answer("✅ Привычка с напоминанием успешно создана!", reply_markup=kb.habits_kb)
